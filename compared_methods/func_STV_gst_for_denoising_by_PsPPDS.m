...
%
% A = (PD O O; I I I; O O Dv)
%
% Algorithm is based on Pock's P-PDS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [HSI_restored, removed_noise, other_result] ...
     = func_STV_gst_for_denoising_by_PsPPDS(HSI_clean, HSI_noisy, params)
...
switch boundary
    case 'Neumann' % Difference operators with Neumann boundary
        ...
        gamma1_U    = gpuArray(single(1./(...
            (precon1_Dv + precon1_Dh)*prod(blocksize)/prod(shiftstep) + 1)));
        ...
        %memory release
        clear precon1_Dv precon1_Dh
        clear precon2_Dv precon1_Dh
        clear zero_para one
    ...
    case 'circulant' % Difference operators with circulant boundary
        ...
        gamma1_U    = gpuArray(single(1./(prod(blocksize)/prod(shiftstep) * 2*2 + 1))); % P*D + I
        gamma1_S    = gpuArray(single(1));
...
for i = 1:maxiter
    ...
    Y1_next = Y1_tmp - gamma2_Y1.*Prox_STV(Y1_tmp./gamma2_Y1, 1./gamma2_Y1, blocksize);
